...
object generic {
  ...
  implicit def optionEncoder[T, F <: ScalarField](implicit
      enc: ScalarEncoder[T, F],
      s: Scalar[T]
  ): Encoder[Option[T], Nullable[F]] = new OptionEncoder[T, F]()
  implicit def arrayEncoder[T, F <: ScalarField](implicit
      enc: ScalarEncoder[T, F],
      s: Scalar[T]
  ): Encoder[Seq[T], CArray[F]] = new ArrayEncoder[T, F]()
  implicit def nestedEncoder[T <: Product, F <: Field](implicit enc: Encoder[T, F]): Encoder[Seq[T], CNested] =
    new NestedEncoder()
  ...
  object auto extends LabelledProductTypeClassCompanion[RowEncoder] {
    object typeClass extends LabelledProductTypeClass[RowEncoder] {
      override def emptyProduct: RowEncoder[HNil] = new Encoder[HNil, Field] {
        override def encode(name: String, value: HNil): Seq[(String, Field)] = Seq.empty[(String, Field)]
      }
      ...
      override def product[H, T <: HList](
          ...
          ch: RowEncoder[H],
          ct: RowEncoder[T]
      ): RowEncoder[shapeless.::[H, T]] = new Encoder[shapeless.::[H, T], Field] {
        override def encode(xname: String, value: shapeless.::[H, T]): Seq[(String, Field)] = {
          ch.encode(name, value.head) ++ ct.encode(name, value.tail)
        }

      }

      override def project[F, G](instance: => RowEncoder[G], to: F => G, from: G => F): RowEncoder[F] =
        new Encoder[F, Field] {
          override def encode(name: String, value: F): Seq[(String, Field)] = {
