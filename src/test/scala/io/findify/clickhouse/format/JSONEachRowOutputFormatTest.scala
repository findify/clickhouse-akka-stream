...
class JSONEachRowOutputFormatTest extends FlatSpec with Matchers {
  ...
  it should "encode simple Row" in {
    val row = Row(Seq("foo" -> Field.UInt32(1), "bar" -> Field.CString("baz")))
  ...
  it should "encode nested objects" in {
    val row = Row(
      Seq(
        "foo" -> Field.UInt32(1),
        "bar" -> CNested(Seq(ScalarRow(Seq("a" -> Int32(1))), ScalarRow(Seq("a" -> Int32(2)))))
      )
    )
  ...
  it should "encode arrays" in {
    val row = Row(Seq("foo" -> Field.UInt32(1), "bar" -> CArray(Seq(Int32(1), Int32(2)))))
