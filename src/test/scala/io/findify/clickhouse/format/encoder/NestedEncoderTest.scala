...
class NestedEncoderTest extends FlatSpec with Matchers {
  ...
  it should "derive for nested rows" in {
    enc.encode("", Hello("a", Seq(Nest("b", 1)))) shouldBe Seq(
      "a" -> CString("a"),
      "b" -> CNested(Seq(ScalarRow(Seq("b" -> CString("b"), "c" -> Int32(1)))))
    )
  }
  ...
  it should "fail for deeply nested rows" in {
    ...
    enc.encode("", Hello2("a", Seq(Nest2("b", Seq(1, 2))))) shouldBe Seq(
      "a" -> CString("a"),
      "b" -> CNested(Seq(ScalarRow(Seq("b" -> CString("b"), "c" -> CArray(Seq(Int32(1), Int32(2)))))))
    )
  }
}
